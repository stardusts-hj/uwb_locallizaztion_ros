	...
	/* Non Cannonical mode                                                                                */
	/*----------------------------------------------------------------------------------------------------*/
        /* Program reads a string from the serial port at 51200 bps 8N1 format                                 */
	/* Baudrate - 51200                                                                                    */
	/* Stop bits -1                                                                                       */
	/* No Parity                                                                                          */
        /*----------------------------------------------------------------------------------------------------*/
...
#include "trilateration.h"
	...
	int GetLocatrion(vec3d *best_solution, int use4thAnchor, vec3d* anchorArray, int *distanceArray);
...
int GetLocation(vec3d *best_solution, int use4thAnchor, vec3d* anchorArray, int *distanceArray)
{
	...
	int main(void)
		...
		/*print the data and change it into real distance (int) */
        for(i=0;i<bytes_read;i++)	 		/*printing only the received characters*/
			printf("%c",read_buffer[i]);
		printf("buffer byte len : %d\n", bytes_read);
			
			

		for(j = 6;j<14;j++){
			printf("%c",read_buffer[j]);   /*get the distance between the tag and A0*/
			distance[j - 6] = read_buffer[j];
			}
		Range_deca[0] = str2int(distance);
		printf("	");


		for(j = 15;j<23;j++){				/*get the distance between the tag and A1*/
			printf("%c",read_buffer[j]);
			distance[j - 15] = read_buffer[j];
			}
		Range_deca[1] = str2int(distance);
		printf("	");

		for(j = 24;j<32;j++){				/*get the distance between the tag and A2*/
			printf("%c",read_buffer[j]);			
			distance[j - 24] = read_buffer[j];
			}
		Range_deca[2] = str2int(distance);
		/*
